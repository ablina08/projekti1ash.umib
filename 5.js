function getWeather() {
    ...
    function displayWeather(data) {
        ...
        const temperature = Math.round(data.main.temp); // Celsius already (units=metric)
        ...
        // The weather icon should already be shown once it's set here.
    ...
    function displayHourlyForecast(hourlyData) {
        ...
        // Slice the next 8 data points (next 24 hours)
        const next24Hours = hourlyData.slice(0, 8); // Display the next 24 hours (3-hour intervals)
      
        next24Hours.forEach(item => {
            const dateTime = new Date(item.dt * 1000); // Convert timestamp to milliseconds
            const hour = dateTime.getHours();
            const temperature = Math.round(item.main.temp); // Celsius already
